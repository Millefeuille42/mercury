...
impl Commands {
	...
	async fn execute<'a>(&self, args: Vec<&str>, channels: IRCCommChannels<'a>, ctx: &mut irc::Context) -> Result<(), Box<dyn Error>> {
		match self {
...
pub async fn execute<'a>(args: Vec<&str>, channels: IRCCommChannels<'a>, ctx: &mut irc::Context) -> Result<(), Box<dyn Error>> {
